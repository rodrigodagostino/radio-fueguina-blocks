...
import { createBlock, getBlockDefaultClassName, registerBlockVariation } from '@wordpress/blocks'
import {
  BaseControl,
  Button,
  FocalPointPicker,
...
function Content({
  ...
  const blockProps = useBlockProps({
    className: classnames({
      ...
      [ `background-position-${ focalPoint.small.x * 100 }-${ focalPoint.small.y * 100 }--sm` ]: mediaId && ( focalPoint.small.x || focalPoint.small.y ),
      [ `background-position-${ focalPoint.medium.x * 100 }-${ focalPoint.medium.y * 100 }--md` ]: mediaId && ( focalPoint.medium.x || focalPoint.medium.y ) && ( focalPoint.medium.x !== focalPoint.small.x || focalPoint.medium.y !== focalPoint.small.y ),
      [ `background-position-${ focalPoint.large.x * 100 }-${ focalPoint.large.y * 100 }--lg` ]: mediaId && ( focalPoint.large.x || focalPoint.large.y ) && ( focalPoint.large.x !== focalPoint.medium.x || focalPoint.large.y !== focalPoint.medium.y ),
      [ `background-size-${ backgroundSize.small }--sm` ]: mediaId && backgroundSize.small,
      [ `background-size-${ backgroundSize.medium }--md` ]: mediaId && backgroundSize.medium && backgroundSize.medium !== backgroundSize.small,
      [ `background-size-${ backgroundSize.large }--lg` ]: mediaId && backgroundSize.large && backgroundSize.large !== backgroundSize.medium,
      [ `background-repeat-${ backgroundRepeat.small }--sm` ]: mediaId && backgroundRepeat.small,
      [ `background-repeat-${ backgroundRepeat.medium }--md` ]: mediaId && backgroundRepeat.medium && backgroundRepeat.medium !== backgroundRepeat.small,
      [ `background-repeat-${ backgroundRepeat.large }--lg` ]: mediaId && backgroundRepeat.large && backgroundRepeat.large !== backgroundRepeat.medium,
                  ...
                  onSelect={ ( media ) => {
                    ...
                    setResponsiveAttribute(
                      ...
                      [
                        ...
                        media.sizes.large ? media.sizes.large.url : media.sizes.full.url,
                      ],
                    )
                  } }
                  ...
                  render={ ({ open }) =>
                    <>
                      <Button
                        ...
                        onClick={ open }
                      >
                        { !mediaId ? __( 'Choose background image', 'radiofueguinablocks' ) : __( 'Replace image', 'radiofueguinablocks' ) }
                      </Button>
                        ...
                        { ( tab ) =>
                          <>
                            ...
                            <SelectControl
                              ...
                              options={ [
                                { label: __( 'Auto', 'radiofueguinablocks' ), value: 'auto' },
                            ...
                            <SelectControl
                              ...
                              options={ [
                                { label: __( 'Repeat', 'radiofueguinablocks' ), value: 'repeat' },
                                { label: __( 'Repeat X', 'radiofueguinablocks' ), value: 'repeat-x' },
              ...
              @media only screen and (min-width: ${ fleximpleblocksPluginData.settings.mediumBreakpointValue }px) {
                ${ mediaUrl.medium ? `
                  .${ defaultClassName }.background-image-id-${ mediaId }--md {
								...
								${ ( !!focalPoint.medium.x || !!focalPoint.small.y ) && ( focalPoint.medium.x !== focalPoint.small.x || focalPoint.medium.y !== focalPoint.small.y ) ? `
...
const Placeholder = ({
  ...
  const createBlocksFromInnerBlocksTemplate = ( innerBlocksTemplate ) => {
    return map( innerBlocksTemplate, ( [ name, attrbts, innrBlcks = [] ] ) => createBlock(
      name,
      attrbts,
      createBlocksFromInnerBlocksTemplate( innrBlcks ),
    ) )
  }
  ...
  const blockProps = useBlockProps({ className: 'radiofueguina-block-frontpage-section--placeholder' })
